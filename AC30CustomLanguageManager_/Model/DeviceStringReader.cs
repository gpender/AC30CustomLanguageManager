...
namespace AC30CustomLanguageManagerApp.Model
{
    public class DeviceStringReader : IDeviceStringReader
    {
        ...
        private IEnumerable<ILanguageStringCollection> GetLanguagesFromStringsSection(XNamespace nameSpace, XDocument doc)
        {
            ...
            bool enCulturePresent = false;
            foreach (var als in availableLanguageStrings)
            {
                string lang = als.Language.ToLower();
                if(lang == "en" || lang == "en-gb" || lang == "english")
                {
                    enCulturePresent = true;
                }
            }
